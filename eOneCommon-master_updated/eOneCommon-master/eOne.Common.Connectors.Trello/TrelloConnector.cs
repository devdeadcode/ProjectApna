...
namespace eOne.Common.Connectors.Trello
{
    public class TrelloConnector : RestConnector
    {
        ...
        public TrelloConnector()
        {
            ...
            // rate limited to 100 requests per 10 second interval for each token
            AddRateLimit(ServiceConnectorRateLimiting.LimitAppliedTo.Account, 100, ServiceConnectorRateLimiting.LimitPeriod.Second, 10);
        ...
        {
            base.Initialise();
            UrlParameters = TupleHelper.CreateTupleStringList("token", Token, "key", Key);
            AddEntities();
            AddCompanies();
        }
