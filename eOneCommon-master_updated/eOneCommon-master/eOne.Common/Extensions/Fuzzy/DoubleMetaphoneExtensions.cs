...
namespace eOne.Common.Extensions.Fuzzy
{
    public static class DoubleMetaphoneExtensions
    {
        ...
        public static string ToDoubleMetaphone(this string input)
        {
            ...
            var last = input.Length - 1; // zero based index

            var workingString = input.ToUpperInvariant() + "     ";

            var isSlavoGermanic = (input.IndexOf(charW) > -1) 
            ...
            if (workingString.StartsWith(StringComparison.OrdinalIgnoreCase, strGN, strKN, strPN, strWR, strPS)) current += 1;
            ...
            while ((metaphoneData.PrimaryLength < 4) || (metaphoneData.SecondaryLength < 4))
            {
                ...
                switch (workingString[current])
                {
                    ...
                    case charG:
                        if (workingString[current + 1] == charH)
                        {
                            ...
                            if (current < 3)
                            {
                                ...
                                if (current == 0)
                                {
                                    metaphoneData.Add(workingString[current + 2] == charI ? strJ : strK);
                                    current += 2;
                                    break;
                                }
                            }
                    ...
                    case charS:
                        ...
                        {
                            //germanic
                            metaphoneData.Add(StringAt(workingString, (current + 1), strHEIM, strHOEK, strHOLM, strHOLZ) ? strS : strX);
                            current += 2;
                            break;
                        }
        ...
        {
            return (self == charA) || (self == charE) || (self == charI) || (self == charO) || (self == charU) || (self == charY);
        }
