...
namespace eOne.Common.Connectors.Database
{
    public abstract class DatabaseConnector : Connector
    {
        ...
        public override string GetData(string queryXml, ConnectorSerializationType type)
        {
        ...
        public override string GetData(ConnectorQuery query, ConnectorSerializationType type)
        {
            var dataSet = GetDataSet(GetQuerySql(query));
            if (dataSet == null) return string.Empty;
            switch (type)
            {
                case ConnectorSerializationType.Json:
        ...
        public virtual string GetTableSql(ConnectorTable table)
        {
        ...
        public virtual string GetJoinType(ConnectorTable.ConnectorTableJoinType type)
        {
            switch (type)
            {
                case ConnectorTable.ConnectorTableJoinType.Inner:
                    return Syntax.Join.Inner;
                case ConnectorTable.ConnectorTableJoinType.Left:
                    return Syntax.Join.Left;
            }
            return string.Empty;
        }
        public virtual string FormatTable(ConnectorTable table)
        {
        ...
        public virtual string FormatField(ConnectorField field, bool useAlias = false)
        {
            string fieldSql;
        ...
        public virtual string GetListSql(string fieldSql, ConnectorField field)
        {
        ...
        public virtual string GetWhereClause(ConnectorQuery query)
        {
            ...
            // set the first conjunctive to none to avoid incorrectly formatted queries
            query.Restrictions[0].ConjunctiveOperator = ConnectorRestriction.ConnectorRestrictionConjunctiveOperator.None;
            
            var whereClause = new StringBuilder();
            foreach (var restriction in query.Restrictions)
            {
                whereClause.Append(GetWhereConjunctive(restriction.ConjunctiveOperator));
                // todo - handle subqueries
                whereClause.Append(GetWhereItem(restriction));
            }
            return string.Format(Syntax.Where.Format, whereClause);
        }
        public virtual string GetWhereConjunctive(ConnectorRestriction.ConnectorRestrictionConjunctiveOperator conjunctive)
        {
            switch (conjunctive)
            {
                case ConnectorRestriction.ConnectorRestrictionConjunctiveOperator.And:
                    return Syntax.Where.And;
                case ConnectorRestriction.ConnectorRestrictionConjunctiveOperator.Or:
                    return Syntax.Where.Or;
            }
            return string.Empty;
        }
        public virtual string GetWhereItem(ConnectorRestriction restriction)
        {
            var field = FormatField(restriction.Field);
            switch (restriction.RestrictionType)
            {
                ...
                case ConnectorRestriction.ConnectorRestrictionType.NotBetween:
                    var value1 = GetWhereValue(restriction.Values[0]);
                    var value2 = GetWhereValue(restriction.Values[1]);
                ...
                case ConnectorRestriction.ConnectorRestrictionType.StartsWithOneOfList:
                    var values = GetWhereValueList(restriction.Values);
                    return string.Format(GetWhereComparison(restriction.RestrictionType), field, values);
            }
            var value = GetWhereValue(restriction.Values[0]);
        ...
        public virtual string GetWhereValueList(List<ConnectorValue> values)
        {
            return string.Join(Syntax.Where.ValueSeparator, values.Select(GetWhereValue).ToList());
        }
        public virtual string GetWhereValue(ConnectorValue value)
        {
            switch (value.Type)
            {
        ...
        public virtual string GetWhereDataValue(ConnectorValue.ConnectorDateValueType dateType)
        {
            switch (dateType)
            {
        ...
        public virtual string GetSortItem(Tuple<ConnectorField, ConnectorQuery.ConnectorQuerySortOrder> sortItem)
        {
