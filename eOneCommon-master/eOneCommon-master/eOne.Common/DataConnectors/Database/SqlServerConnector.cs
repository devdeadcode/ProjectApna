...
namespace eOne.Common.DataConnectors.Database
{
    public abstract class SqlServerConnector: DatabaseConnector
    {
        ...
        {
            string objectCountSql = "select count(*) as ObjectCount from {0}..sysobjects where name = {1} and xtype = {2}";
        ...
        {
            string fieldListSql = string.Format("select name from {0}..syscolumns where id in (select id from {0}..sysobjects where xtype in ('U', 'V') and name = '{1}')", database, table);
        ...
        public override string GetTableSql(DataConnectorTable table)
        {
