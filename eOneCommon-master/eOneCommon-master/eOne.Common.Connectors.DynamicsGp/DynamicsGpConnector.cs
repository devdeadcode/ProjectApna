...
using eOne.Common.DataConnectors.Database;
...
namespace eOne.Common.Connectors.DynamicsGp
{
    public class DynamicsGpConnector : SqlServerConnector
    {
        ...
        public DynamicsGpConnector()
        {
            ...
            var accountIndexFieldType = new DataConnectorFieldType(FieldTypeIdAccountIndex, "Account index", typeof(string))
            {
                SqlFunction = "isnull((select top 1 rtrim(ACTNUMST) from {1}..GL00105 where ACTINDX = {0}),'')"
            };
            FieldTypes.Add(accountIndexFieldType);
            var noteIndexFieldType = new DataConnectorFieldType(FieldTypeIdNoteIndex, "Note index", typeof(string))
            {
        ...
        public override void Initialise()
        {
            ...
            {
                var group = new DataConnectorEntityGroup(module.Id, module.Name);
